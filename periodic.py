...
ELEM = {'h':1, 'he':2, 'li':3, 'be':4, 'b':5, 'c':6, 'n':7, 'o':8, 'f':9,
        ...
        'cn':112, 'uut':113, 'uuq':114, 'uup':115, 'uuh':116, 'uus':117,
        'uuo':118}
...
def _parse_text(txt):
    ...
    for i in range(len(txt)):
        if i+1 < len(txt) and txt[i:i+1] in ELEM:
            if len(txt[i+1:]) != 0:
                return (ELEM[txt[i:i+1]], _parse_text(txt[i+1:]))
            else:
                return ELEM[txt[i:i+1]]

        if i+2 < len(txt) and txt[i:i+2] in ELEM:
            if len(txt[i+2:]) != 0:
                return (ELEM[txt[i:i+2]], _parse_text(txt[i+2:]))
            else:
                return ELEM[txt[i:i+2]]

        if i+3 < len(txt) and txt[i:i+3] in ELEM:
            if len(txt[i+3:]) != 0:
                return (ELEM[txt[i:i+3]], _parse_text(txt[i+3:]))
            else:
